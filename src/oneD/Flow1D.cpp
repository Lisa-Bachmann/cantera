...
namespace Cantera
{
    ...
    Flow1D::Flow1D(ThermoPhase *ph, size_t nsp, size_t points) : Domain1D(nsp + c_offset_Y, points),
                                                                 m_nsp(nsp)
    {
        m_points = points;
                                ...
                                double rdt, size_t jmin, size_t jmax)
    {
        ...
        if (jmin == 0)
        { // left boundary
            rsd[index(c_offset_U, jmin)] = -(rho_u(x, jmin + 1) - rho_u(x, jmin)) / m_dz[jmin] - (density(jmin + 1) * V(x, jmin + 1) + density(jmin) * V(x, jmin));
        ...
        { // right boundary
            ...
            { // zero gradient, same for unstrained or free-flow
        ...
        { // "axisymmetric-flow"
            ...
            { // interior points
                ...
                rsd[index(c_offset_U, j)] = -(rho_u(x, j + 1) - rho_u(x, j)) / m_dz[j] - (density(j + 1) * V(x, j + 1) + density(j) * V(x, j));
        ...
        { // "unstrained-flow" (fixed mass flow rate)
            for (size_t j = j0; j <= j1; j++)
            {
                              ...
                              double rdt, size_t jmin, size_t jmax)
    {
        ...
        for (size_t j = j0; j <= j1; j++)
        { // interior points
            rsd[index(c_offset_V, j)] = (shear(x, j) - lambda(x, j) - rho_u(x, j) * dVdz(x, j) - m_rho[j] * V(x, j) * V(x, j)) / m_rho[j];
                            ...
                            double rdt, size_t jmin, size_t jmax)
    {
        ...
        for (size_t j = j0; j <= j1; j++)
        {
            if (m_do_energy[j])
            {
                ...
                rsd[index(c_offset_T, j)] = -m_cp[j] * rho_u(x, j) * dTdz(x, j) - conduction(x, j) - sum;
                             ...
                             double rdt, size_t jmin, size_t jmax)
    {
        ...
        for (size_t j = j0; j <= j1; j++)
        {
            for (size_t k = 0; k < m_nsp; k++)
            {
                ...
                rsd[index(c_offset_Y + k, j)] = (m_wt[k] * m_wdot(k, j) - convec - diffus) / m_rho[j] - rdt * (Y(x, k, j) - Y_prev(k, j));
